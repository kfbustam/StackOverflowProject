...
class Question {
        ...
        static getQuestionsByFilter = async (data) => {
                ...
                try {
                        ...
                        {       
                                questions = await QuestionModel.find({'isApproved':true}).populate('tags').populate('user').sort({"updatedAt":1});             
                        }
                        else if(filter === "Hot")
                        {
                                questions = await QuestionModel.find({'isApproved':true}).populate('tags').populate('user').sort({"todayview":-1})
                        }
                        else if(filter === "Score")
                        {
                                questions = await QuestionModel.find({'isApproved':true}).populate('tags').populate('user').sort({"score":-1})
                        }
                        else if (filter === "Unanswered")
                        {
                                questions = await QuestionModel.find( {$and:[{'answer_id.0':{ $exists:false  }}, {'isApproved':true}]}).populate('tags').populate('user');
